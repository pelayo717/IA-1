...
(defun f-h-time (state sensors)
  (first (f-h-aux state sensors)))

(defun f-h-price (state sensors)
  (second (f-h-aux state sensors)))

;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;
;; Function f-h: Exercise 1
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
(defun f-h-aux (state sensors)
  (cond
    ((null sensors) nil)
    ((equal (first (first sensors)) state)
     (second (first sensors)))
    (t (f-h-aux state (rest sensors)))))
...
(defun navigate (state lst-edges cfun name &optional forbidden)
  (if (or (null state) (null lst-edges))
      nil
      (mapcar #'(lambda (x)
                  (make-action
                   :name name
                   :origin state
                   :final (second x)
                   :cost (funcall cfun (third x))))
              (remove-if
               #'(lambda (x)
                   (or (equal (second x) (first forbidden))
                       (not (equal state (first x)))))
               lst-edges))))
...
(defun navigate-canal-time (state canals)
  (navigate state canals #'(lambda (cost) (first cost)) 'navigate-canal-time nil))

(defun navigate-canal-price (state canals)
  (navigate state canals #'(lambda (cost) (first (rest cost))) 'navigate-canal-price nil))
...
(defun navigate-train-time (state trains forbidden)
 (navigate state trains #'(lambda (cost) (first cost)) 'navigate-train-time forbidden))
  
(defun navigate-train-price (state trains forbidden)
  (navigate state trains #'(lambda (cost) (first (rest cost))) 'navigate-train-price forbidden))
...
(defun f-goal-test (node destination-cities mandatory-cities)
  (and (member (node-state node) destination-cities)
       (cities-in-path-p (collect-states (node-parent node))
                         mandatory-cities)))

;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; Aux-rec functions -- Exercise 3
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
(defun cities-in-path-p (path cities)
  (equal (length cities)
...
(defun collect-states (parent &optional states)
  (if (null parent)
      states
      (collect-states (node-parent parent)
                      (cons (node-state parent)
                            states))))
...
;;
(defun f-search-state-equal (node1 node2 &optional mandatory)
  (when (equal (node-state node1)
               (node-state node2))
    (let ((path1 (collect-states node1))
          (path2 (collect-states node2)))
      (or (null mandatory)
          (path-contains-same-cities-p path1 path2 mandatory)))))

;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; Aux-rec functions -- Exercise 4
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
(defun path-contains-same-cities-p (path1 path2 mandatory)
  (every #'(lambda (x) (equal x t))
         (mapcar #'(lambda (mandatory-city)
                     (let ((is-in-path1 (member-p mandatory-city path1))
                           (is-in-path2 (member-p mandatory-city path2)))
                       (or (and is-in-path1 is-in-path2)
                           (and (not is-in-path1)
                                (not is-in-path2)))))
                 mandatory)))

(defun member-p (elt lst)
...
(defun expand-node (node problem)
  (mapcar
   #'(lambda (action)
       (let ((gvalue (+ (node-g node)
                        (action-cost action)))
             (hvalue (funcall (problem-f-h problem)
                              (action-final action))))
         (make-node
          ...
          :h hvalue
          :f (+ gvalue hvalue))))
   (create-actions node problem)))

;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; Aux-rec functions -- Exercise 6
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
(defun create-actions (node problem)
...
(defun insert-nodes-strategy (nodes lst-nodes strategy)
  (let ((node-compare-p (strategy-node-compare-p strategy)))
    (cond
      ...
      ((null lst-nodes)
       (insert-nodes-strategy (rest nodes)
                              ...
                              strategy))
      ;; insertar N nodos
      (t (insert-nodes-strategy (rest nodes)
...
;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;
(defun node-compare-a-star-p (node1 node2)
  (when (and (not (null node1))
             (not (null node2)))
    (<= (node-f node1)
        (node-f node2))))
...
(defparameter *A-star*
  (make-strategy :name 'A-star
                 :node-compare-p #'node-compare-a-star-p))
...
;; Aux-- Exercise 9
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
(defun expand-it (node closed-lst)
  (if (null closed-lst)
      T
      (let ((fst (first closed-lst)))
        (if (and (f-search-state-equal node fst *mandatory*)
                 (>= (node-g node) (node-g fst)))
            NIL
            (expand-it node (rest closed-lst))))))

(defun graph-search-aux (problem strategy open-nodes closed-nodes)
  (let ((first-open (first open-nodes)))
    (cond
      ((null open-nodes))
      ;; si el primer nodo de la lista de abiertos es el objetivo devolver y terminar
      ((f-goal-test first-open
                    *destination*
                    *mandatory*)
       first-open)
      ;; si el primer nodo de la lista de abiertos no esta en la lista de cerrados
      ((expand-it first-open closed-nodes)
       (graph-search-aux problem
                         strategy
                         (insert-nodes-strategy (expand-node first-open problem)
                                                (rest open-nodes)
                                                strategy)
                         (cons first-open closed-nodes)))
      (t
       (graph-search-aux problem
                         strategy
                         (rest open-nodes)
...
(defun graph-search (problem strategy)
  (let ;; llamo open-nodes a la lista con el estado inicial
      ((open-nodes (list
                    (make-node
                     :state (problem-initial-state problem)))))
    ;; inicializo closed-nodes a nil porque no hay nodos explorados inicialmente
...
;;;
;*** solution-path ***
(defun visited-node (node)
  (unless (null node)
    ;; si el nodo no tiene padre
    (if (null (node-parent node))
        ;; formo la lista con el nodo actual
        (list (node-state node))
        ;; meto en la primera posicion el nodo actual con sus anteriores
        (cons (node-state node)
              (visited-node (node-parent node))))))


(defun solution-path (node)
  (reverse (visited-node node)))

;*** action-sequence ***
; Visualize sequence of actions
(defun action-node (node)
  (unless (or (null node) (null (node-action node)))
    (cons (node-action node)
          (action-node (node-parent node)))))
(defun action-sequence (node)
  (reverse (action-node node)))
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;
;; Test exercise 10
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;; 
;;;    BEGIN Exercise 11: Depth & Bredth
;;;

(defun depth-first-node-compare-p (node1 node2)
  T)
...
(defun breadth-first-node-compare-p (node1 node2)
  NIL)
...
;; -> (MARSEILLE TOULOUSE LYON ROENNE NEVERS LIMOGES ORLEANS NANTES BREST ST-MALO PARIS 
;;     ST-MALO BREST NANTES TOULOUSE LYON NANCY REIMS CALAIS)
